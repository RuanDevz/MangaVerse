...
import type { Manga, Chapter, ChapterPages } from '../types/manga';
...
export function MangaDetail() {
  ...
  React.useEffect(() => {
    const fetchManga = async () => {
      ...
      try {
        ...
        // Ordena os capítulos em ordem decrescente com base no número do capítulo
        const sortedChapters = chaptersResponse.data.sort((a, b) => {
  ...
  const loadChapter = async (chapter: Chapter) => {
    ...
    try {
      const response: ChapterPages = await getChapterPages(chapter.id);
