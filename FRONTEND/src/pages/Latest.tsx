...
export function Latest() {
  ...
  React.useEffect(() => {
    const fetchMangas = async () => {
      try {
        const response = await getMangaList({ order: { latestUploadedChapter: 'desc' } });
        if (response.result === 'ok' && Array.isArray(response.data)) {
          setMangas(response.data);
        } else {
          setError('Failed to load latest manga list');
        }
      } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-24">
      <h1 className="text-3xl font-bold mb-8 dark:text-white">Latest Manga</h1>
      {mangas.length > 0 ? (
